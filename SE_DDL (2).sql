...
CREATE TABLE user_details (emp_id char(8), name varchar(25) not null, phone_number char(10) not null, date_of_birth varchar(20) not null, password varchar(30) not null, gender varchar(6) not null, branch_id char(7), dept_id char(7), security_question varchar(50),security_answer varchar(20),PRIMARY KEY(emp_id));
...
ALTER TABLE user_details ADD FOREIGN KEY (branch_id) REFERENCES branch(branch_id) on delete set NULL;

ALTER TABLE user_details ADD FOREIGN KEY (dept_id) REFERENCES department(dept_id) on delete set NULL;
